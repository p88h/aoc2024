...
pub fn expand2(from: u8, to: u8) []const []const u8 {
    ...
    if (to == '0') return switch (from) {
        ...
        '7' => &.{">vvvA"}, //  "v>vvA" ; "vv>vA"
        '8' => &.{"vvvA"},
        '9' => &.{ "<vvvA", "vvv<A" }, // "vv<vA"; "v<vvA"
        'A' => &.{"<A"},
        else => &.{"A"},
    };
    if (from == 'A') return switch (to) {
        '1' => &.{"^<<A"}, // ; "<^<A"
        '2' => &.{ "^<A", "<^A" },
        '3' => &.{"^A"},
        '4' => &.{"^^<<A"}, // "^<^<A" ; "^<<^A", "<^<^A", "<^^<A"
    ...
    // inverse
    if (to == 'A') return switch (from) {
        '1' => &.{">>vA"}, // ; ">v>A"
        '2' => &.{ ">vA", "v>A" },
        '3' => &.{"vA"},
        '4' => &.{">>vvA"}, // ">v>vA" ; "v>>vA", "v>v>A", ">vv>A"
        '5' => &.{ ">vvA", "vv>A" }, // "v>vA"
        '6' => &.{"vvA"},
        '7' => &.{">>vvvA"}, // ">v>vvA", "vv>>vA", "v>v>vA", ">vv>vA",  "v>vv>A" ;  "v>>vvA", "vv>v>A", ">vvv>A"
        '8' => &.{ ">vvvA", "vvv>A" }, // "v>vvA" ;  "vv>vA"
        '9' => &.{"vvvA"},
        else => &.{""}, // not really
    };
    return switch (from) {
